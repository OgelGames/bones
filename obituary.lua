...
local function show_obituary_formspec(stack, player)
	...
	-- Fail-safe position parsing to prevent string.format nil errors
	local pos = core.string_to_pos(meta:get_string("pos"))
...
-- Safely wrap obituary features instead of using an early file return
if bones.obituary then
		...
		for _,list in pairs(items) do
			for _,stack in ipairs(list) do
				...
				if count > 0 then
					...
					desc = (def and def.original_description) or stack:get_short_description()
